...
## 2026-07-17 — Texnomart Promo: «Группа A» — сводный тракер комментариев

### A Client Tracker's Statuses Are a Snapshot — Diff Each ⛔ Against the Code Before Re-Implementing
- The tracker's «Статус выполнения» column was dated 08.07, but the «8-я»/«6-я» rounds landed 09–10.07 — so 4 items the client marked «Не сделано» (sidebar collapse, rejected-plan marker, plan editing, send checkboxes) **already worked in the mock**, and 2 more were half-closed. Blindly implementing the tracker top-to-bottom would have re-built existing features (and possibly regressed them). Lesson: for every ⛔/🟡 item, first locate the feature in code (grep the Russian UI strings from the комментарий — they're near-unique anchors), classify as {real gap / already done / done-elsewhere-not-deployed}, and only then scope work. Half the value of the pass was the classification table (`docs/promo_feedback_tracker.md`), not the code.

### Sticky-BOTTOM Viewport Scrollbar = a 4th Synced Scroller as the Card's LAST Child; `overflow-clip` on the Card Doesn't Trap It
- The client's «горизонтальный скролл как в Excel» = a scrollbar pinned to the bottom of the VISIBLE area while the page scrolls. Implementation that worked with the existing 3-scroller Pattern-F sync (top strip + header + body): add a `sticky bottom-0 z-30` strip as the **last child of the Card** — a spacer `width: frozenW` + a track `width: scrollW, height: 1px` — and extend the idempotent `syncScroll` loop to 4 refs. Two subtleties: (1) the Card's `overflow-clip` does NOT establish a scroll container (unlike `overflow-hidden`+size constraints for scrolling), so sticky positioning resolves against the AppShell `<main>` scrollport — same mechanism the sticky header already relied on; (2) hide the body pane's own native h-scrollbar (`[scrollbar-width:none] [&::-webkit-scrollbar]:hidden`, keep `overflow-x-auto`) or you get a doubled scrollbar at the table's true bottom.

### Formatted Display Ids: Convert at Render (or in Display-Only Builder Fields), Keep the Full Id for Every Key/Param/Lookup — and Make the Formatter Idempotent
- Rolling «26-N» out to 18 files is safe only if the formatted string NEVER flows into identity: React keys, route params, `Map`/`Set` keys, filter matching, deep-links all keep `PR-/UN-`. Practical seams that would have broken silently: multi-select filter options must format only the **label** (`no`) while the option **value** stays the raw id (3 pre-existing sites had it backwards); a builder-level conversion (`ControlPoint.promoNo`) is OK only after tracing every consumer is display/export — keep a parallel raw `campaignId` for grouping; a composite id (`campaignId~kmId`) must be split before formatting (the `formatPromoNo` regex requires the full `XX-YYYY-NNN` shape, so it no-ops on garbage — deliberate idempotence that also makes double-formatting harmless); and a dual-use variable feeding both a dialog display AND the persisted row id must be split into `displayNoLabel` vs `displayNo`.

### Persisting a Multi-Slice React Lifecycle: One Snapshot Read → N Lazy Initializers → One Write-Through Effect
- `PlanMode`'s 7 lifecycle `useState`s hydrate from ONE `getPlanState()` read captured in a `useState(() => …)` (not a useEffect — no seed flash, no N JSON.parses, no cross-slice inconsistency), each slice's initializer falling back to its seed default so absent/corrupt storage degrades to pre-persistence behavior byte-for-byte. ONE effect with an exhaustive dep list persists the assembled snapshot — every mutation path (create/edit/delete/send/approve/reject/reset) flows through the slices, so resets persist for free. UI-transient state (selection, dialog-open, edit target) deliberately stays out. Dates: serialize ISO, revive on read (the `notifications-store` idiom). Field-level defensive parsing (`Array.isArray`/`isRecord` per field) beats whole-blob validation — partial corruption degrades per-field instead of nuking the snapshot.

---

## 2026-07-22 — Texnomart Promo: «7-я часть» client feedback

### A "Re-Enter the Review Stage" Guard Keyed Only to Terminal States Strands the Post-Return Cycle — Enumerate the Non-Reviewer States, Not Just the Ends
- `sendSelected` reset the plan to «На согл. с КД» only `if (isApproved || isRejected)`. But «Вернуть на доработку» parks the plan at **«На обсуждении»** (a *marketing* stage, neither terminal nor reviewer), so re-sending the reworked drafts kept the plan there — the КД never regained an actionable queue and the client's reject→return→resend→reject history cycle (§9.3) was unreachable. The QA rounds never caught it because each round tested reject OR return, never the full loop back through send. Fix: re-open the chain whenever the plan is NOT at a reviewer stage — `isApproved || isRejected || currentActor === PLAN_EDITOR`. Lesson: when a state machine has {owner stages, reviewer stages, terminal states}, a transition guard written as "if terminal" silently excludes the owner stages that ALSO need the transition; enumerate by "who acts here", and QA the full round-trip cycle, not each edge once.

### Before Building a Pinned-Bottom Scrollbar, Check What the Scroll Ancestor Already Is — Viewport-Bounded Containers Give It for Free
- The «apply the Excel bottom scrollbar to ALL wide tables» ask looked like 6 band-conversions; it was only 3. The «План акций» `<table>` (no inner overflow wrapper, per the sticky-header lesson) overflows into the AppShell `<main>` — and since `<main>` is a viewport-height flex child, its horizontal scrollbar ALREADY renders at the viewport bottom edge, always visible. Likewise the audit tables scroll inside `max-h-[calc(100vh-…)]` boxes whose own scrollbar can't leave the viewport. Only tables whose h-scrollbar sat at the bottom of tall in-flow content (the 3 band tables) needed the synced `sticky bottom-0` track. Lesson: "pinned to the visible area" is a property of the scroll container's geometry, not something to bolt on per-table — map each table's actual scroll ancestor first; converting the plan table to a band layout would have been days of churn for zero visible change.

### 2-Line Clamp Inside Fixed-Height Pattern-F Rows: Verify the Arithmetic per Surface, Put the Idiom in a Local `CLAMP2` Const
- Replacing `truncate` with the short calendar's `[display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2] overflow-hidden leading-tight` is safe in a dual-pane table ONLY because 2 × ~17px (text-sm leading-tight) fits every fixed height in play (56px full-calendar rows, 52px report rows, 44px gift sub-rows/headers) — the row heights themselves must NOT change or the panes desync. Numeric/date/check cells keep nowrap (clamping them is noise); in the reports table the wrap is gated on `f.kind === "text"`. Watch for `truncate` sitting on a flex CONTAINER (reports header/body cells) — it wasn't truncating children anyway; replace with `overflow-hidden` on the container + the clamp on the inner span.
...
## 2026-07-31 — 10-я часть, Волна 1: «не работает у клиента» = чаще всего off-screen Radix-контент

### DropdownMenu под shared `<Button>` asChild НЕ «degrades gracefully» — он рендерится за экраном; это источник клиентских «кнопка не работает»
- Ранний вывод (2026-06-09), что без ref ломается только Popover, а DropdownMenu «обычно работает», опровергнут: два подтверждённых клиентских бага 10-й части — «Экспорт» плана акций (меню открывалось off-viewport → «файл не скачивается», R29.3) и меню действий в `/users` (rect `top:-315` → «Сбросить пароль недоступно», R69.2). Симптом в Playwright: клик по пункту падает с «element is outside of the viewport», при этом aria-снапшот меню выглядит нормальным.
- Диагностика за одну команду: открыть меню и проверить `menu.getBoundingClientRect()` — отрицательные координаты = ref-потеря анкора. Фикс всегда один: нативный `<button className={cn(buttonVariants({...}))}>` под `asChild`. После фикса стоит грепнуть весь проект на `(DropdownMenu|Popover|Tooltip|Sheet)Trigger asChild>\s*\n\s*<Button` (multiline) и зачистить все сайты разом.
- Когда клиент пишет «кнопка отображается, но ничего не происходит» — СНАЧАЛА проверить эту пару (asChild + shared Button) и DismissableLayer-самозакрытие, потом уже искать логические баги: в Волне 1 три из шести «багов» оказались именно этим.

### Контролируемый Dialog, открываемый обычной кнопкой, обязан открываться через setTimeout(…, 0) — рецидив в PlanMode
- Урок S2 Phase 3 повторился в `PlanMode.openCreate/openEdit` («Создать строку плана»): реальный pointer-клик открывал и мгновенно закрывал диалог (DismissableLayer ловит тот же клик как outside-interaction), а JS `el.click()` в ранних QA-прогонах его НЕ воспроизводил — поэтому баг пережил три раунда проверок. Правило: любой `setState(open)` из onClick кнопки, не являющейся `DialogTrigger`, — только деферரённый; и в QA кликать реальным поинтером (Playwright browser_click), а не `element.click()`.

### Персистентный стор с tombstone-набором + автонумерация от «видимых» строк = невидимые новые строки
- `nextPlanPromoNo` считал следующий номер по видимым строкам, а `deletedIds` (tombstones) переживают reload: удалил черновик → создал новый → номер переиспользован → новая строка мгновенно отфильтрована как «удалённая» (клиентское «создал строку — её нет в таблице», R31.4). Правило: пул автонумерации обязан включать tombstone-ids, а create — дефенсивно вычищать свой id из tombstone-набора.

---

## 2026-07-31 — 10-я часть, Волна 2: построчная модель статусов полного календаря

### Новую спец-маршрутизацию в общем edit-обработчике проверять по ВСЕМ источникам правок, а не только по целевому
- `onEdit` полного календаря стал роутить правку согласованной кампании в `line.pending` (Блок 2 — таблица показывает согласованные данные). Написано с прицелом на data-поля КМ (цена/остаток/прогноз), но `onEdit` — единственный канал правок для ВСЕХ ячеек: чекбоксы «В рекламу (выбрано маркетингом)»/«рекомендация КМ» и выбор подарков тоже идут через него. В результате на согласованной акции чекбокс маркетинга (§7.2) переставал применяться и создавал ложное «повторное согласование». `/code-review` (high) это поймал; фикс — роутить в pending только если патч трогает ключ из `TRACKED_FIELD_LABEL` (data-поля), а adv-флаги/подарки применять напрямую. Правило: добавляя условную ветку в разделяемый обработчик (edit/submit/click), грепни ВСЕ вызовы обработчика и прогони каждый источник, особенно чекбоксы и не-текстовые поля — билд (transpile-only) такое не ловит.

### `LineStatus`-деривация: держать её чистой функцией `(campaign, line) → status` с явным приоритетом, а не размазывать по грид-классам
- Раньше состояние строки складывалось из 5 сигналов (`campaign.status`, `line.rejected`, `removalPending`/`removed`, `changedCells`, `pending1CCheck`) прямо в className грида. Единый `lib/full-calendar-status.ts` с `lineDisplayStatus` (порядок: removed → исключение → отклонённая правка → pending → отмена → campaign.status/rejected) сразу дал: единый построчный фильтр «Все статусы», синтетику «На согласовании (общее)» одним предикатом, «светло-оранжевая только для повторных действий» одним `isRepeatActionPending`, и переиспользование в панели/грид/странице без дублирования логики. Каждую клиентскую «плашку статуса» гоняем через ОДНУ деривацию — иначе таблица, фильтр и панель разъезжаются.

### Pattern-F: подпись/капшн внутри ячейки одной панели надо компенсировать в `lineHeightPx` (обе панели), иначе строки разъезжаются
- Добавление капшна «Подарок на выбор» в gift-колонку scroll-панели (R44) означало +1 под-строку по высоте. Высоту строки задаёт общий `lineHeightPx(line, isChoice, editable)`, применяемый к ОБЕИМ панелям (frozen + scroll). Забыть прибавить `captionRow` в расчёт = frozen-панель короче scroll-панели → построчный разъезд. Правило Pattern F остаётся: любая variable-height добавка в одной панели обязана быть в общей формуле высоты.

---

## 2026-08-03 — 10-я часть, Волна 3: карточка повторного согласования

### Очередь, выведенная из статуса сущности, не увидит новый класс работы — выводите её из артефактов
- `buildReviewItems` строил заявки по `campaign.kmStatuses[kmId]`, ожидающим проверяющего. Волна 2 научила КМ править/добавлять/исключать позиции в УЖЕ согласованной акции (`line.pending`), но её `kmStatus` терминальный — значит новых заявок не возникало ни одной, и половина построенной волной 2 модели была недостижима («workflow дремлет» так и стояло в HISTORY). Правило: когда добавляете новый вид работы, проверьте, из чего строится очередь. Если из статуса-агрегата — новая работа в неё не попадёт; выводите заявки из самих артефактов (строк с неразрешённым действием) и группируйте по владельцу артефакта (`line.kmId`), а не по списку участников акции — позицию мог добавить КМ, которого в `participatingKmIds` нет.

### Число стадий согласования выбирается по тому, что происходит с данными, а не по симметрии с основным потоком
- Соблазн был переиспользовать цепочку Старший КМ → КД для повторных правок. Но согласование повторного действия ПРИМЕНЯЕТ новые значения (§16), поэтому первая же аппрува очистила бы `pending` — и КД получил бы карточку, в которой нечего решать, либо пришлось бы городить промежуточное состояние «одобрено старшим, но ещё не применено». Одностадийное решение на этапе КД честнее и по смыслу (промо уже отправлено смежным отделам — правка к нему это уровень КД). Правило: если действие мутирует данные, лишняя стадия требует ещё одного состояния «решено, но не применено» — сначала посчитайте, готовы ли вы его вводить.

### Дифф, который потом надо ПРИМЕНИТЬ, обязан хранить сырое значение, а не только отформатированные «Было/Стало»
- `LinePendingChange.fields[]` из Волны 2 хранил `{was: "16%", now: "18%"}` — этого хватало показать панель, но не хватало применить изменение при согласовании: обратный разбор «18%» / «7 640 000 сум» — угадайка. Добавлено опциональное `value?: unknown` (пишет `mergePendingChange` прямо из патча) + разбор строки как резерв для старых сидов. Правило: как только у диффа появляется потребитель «применить», сырое значение кладём рядом с форматированным сразу — переформатирование в обе стороны всегда теряет данные.

### Два экрана со своими стор-состояниями синхронизируются чистым `apply*(items)` на посеве, а не подъёмом состояния в провайдер
- Карточка согласования и полный календарь держат строки в собственных сторах (useMemo и useReducer-Map). Чтобы решение проверяющего было видно на обоих, достаточно было персистентного словаря решений + чистой `applyLineDecisions(lines)`, которую ОБА экрана применяют в момент посева. Никакого общего провайдера, никакого рефактора `FullCalendarPage`. Правило для мока: общий факт → persist + чистая функция свёртки на входе каждого потребителя; подъём состояния оставляем для случаев, когда нужна реактивность между экранами в пределах одного рендера.

### Grid-ребёнок с таблицей внутри обязан получить `min-w-0` — иначе `min-width:auto` растянет карточку за экран
- На 390px вся карточка `/approvals/:id` уезжала вправо (`scrollWidth` 928 при вьюпорте 390), хотя таблица честно лежала в `overflow-x-auto`. Причина не в таблице: `<Card>` — ребёнок `grid lg:grid-cols-[1fr_340px]`, а у grid/flex-детей `min-width: auto`, поэтому контейнер не может стать уже min-content-ширины таблицы и распирает страницу. Фикс — `min-w-0` на карточке (и на текстовом блоке заголовка рядом со `shrink-0`-кнопкой). Проверка одной командой: `document.documentElement.scrollWidth` против `window.innerWidth` — расхождение = где-то потерян `min-w-0`, а не «таблица широкая».

---

## 2026-08-04 — 10-я часть, Волна 4: циклы согласования плана и удаление через согласование

### `overflow-clip` на карточке молча отменяет замысел «широкая таблица переполняется в скролл страницы»
- «Группа A» V2-13 сознательно убрала у таблицы плана внутренний `overflow-x-auto` (он перехватывает `position: sticky` у шапки, плюс по CSS-квирку включает и вертикальный скролл), рассчитывая, что переполнение уйдёт в собственный горизонтальный скролл `<main>` — тот ограничен высотой вьюпорта, поэтому его полоса всегда видна. Позже Волна 1 добавила на `<Card>` класс `overflow-clip` ради скруглений закреплённой снизу полосы действий — и переполнение стало **обрезаться, не доходя до скролла страницы**. Симптом ровно тот же, что у off-screen Radix-меню Волны 1: «кнопка отображается, но нажать нельзя» (реальный указатель до неё не доходит, `elementFromPoint` возвращает не ту ноду). Клиент такое пишет как «кнопка не работает».
- Правило: `overflow-clip`/`hidden` на предке широкой таблицы **несовместим** с решением «пусть переполняется в скролл страницы». Если клип нужен только ради скруглений одного элемента — вешайте его **на этот элемент** (здесь: `overflow-clip rounded-b-xl` на саму sticky-полосу), а не на общий контейнер. Проверка — не на глаз, а `getBoundingClientRect()` самой правой кнопки против `window.innerWidth` плюс реальный клик.
- Второй слой урока: расширение таблицы (у нас `min-w` 1200→1320 и безусловная колонка действий 240px) **не создало** проблему, а сделало её видимой — обрезание было и раньше, просто на ~80px меньше. Меняя ширину таблицы, проверяйте крайнюю правую колонку на самом узком поддерживаемом десктопе, а не только то, что «влезло по ширине карточки».

### Колонка, которую рисуют из сида, не покажет живых решений — сверяйте источник ячейки с источником состояния
- R29.5 («не отображается дата согласования и признак срока») был не «забыли добавить поле», а расхождение источников: колонки КД/ОД рендерились из `getPlanApproval` (сид), тогда как живые решения сессии лежали в `decisions` и попадали только в бейдж «Статус строки». У строк вообще без сид-записи там стояли прочерки при любом состоянии. Лечится не добавлением поля, а **единой деривацией** `(строка, журнал) → StageCellData`, внутри которой сид — лишь fallback; таблица, боковая панель и CSV зовут одну и ту же функцию и поэтому не могут разъехаться (тот же приём, что `audit-control.ts` в E-3 и `full-calendar-status.ts` в Волне 2).
- Диагностический вопрос при любой жалобе «поле пустое / не обновляется»: *откуда эта ячейка берёт значение и где живёт состояние, которое пользователь только что изменил?* Если это два разных места — баг структурный, а не косметический.

### Единица измерения срока — часть контракта ячейки, а не деталь рендера
- На одном экране соседствуют дедлайн в **календарных** днях (отправка плана за 60 дн. до старта) и SLA согласования в **рабочих** (3 раб. дн. на этап), а бейдж печатал безликое «Просрочка +N дн.». Пока обе цифры считались из сида, это сходило с рук; как только появилась живая арифметика, «+2 дн.» стало неоднозначным. Решение: `unit: "cal" | "work"` внутри самой структуры данных ячейки, а подпись выводится из него — тогда единица не может разойтись с применённой арифметикой. Это же прямо требует конвенция проекта («дедлайны — календарные, SLA согласования — рабочие»), но до этой волны она соблюдалась только в подписях колонок, а не в значениях.

### Добавляя историю поверх состояния, материализуйте «нулевой» элемент из существующего источника
- Строки, отправленные до появления журнала (сид `PLAN_APPROVALS`), не имели ни одного цикла. Если бы первое живое решение открывало цикл «сейчас», SLA КД считался бы от момента клика проверяющего — то есть всегда «в срок». `ensureOpenCycle` материализует цикл №1 с `marketing.sentAt` **из сида**, и срок считается от реальной отправки. Правило: у любой новой истории есть предыстория — при первой записи достройте её из того источника, который отвечал за состояние раньше, иначе первые метрики окажутся систематически смещёнными.

### Аффорданс, включённый в задаче N, но безопасный только с задачи N+3, — это мёртвая кнопка на всём промежутке
- План в задаче про таблицу отдавал «Удалить» всем строкам, хотя развилку «согласованную строку удалять через согласование» вводила задача тремя шагами позже; до неё обработчик молча выходил по `return`. Пред-полётный скан плана это поймал, гейт был оставлен на месте и снят в той же задаче, что сделала кнопку осмысленной. Правило при нарезке плана: **включение аффорданса и появление его поведения обязаны лежать в одной задаче** — иначе между ними живёт ровно тот баг, на который клиент жалуется как «нажимаю — ничего не происходит».

---

## 2026-08-05 — 10-я часть, Волна 5 (блоки 5A «Отчёты» и 5B «Уведомления»)

### Клиентский комментарий — это снимок ПРОШЛОГО состояния; сверяйте его с кодом до того, как что-то менять
- В блоке 5A из десяти подпунктов «не реализовано» девять оказались сделанными: плашки «Добавлено/Изменено/Исключено», счётчики в шапке, «Было/Стало», «Только изменения», история версий, ознакомление по пользователям — всё построено в E-1, а комментарий писался раньше. Реальных пробелов было три. То же в 5B: «Сбросить пароль недоступно» починено ещё Волной 1.
- Правило: первый шаг работы с раундом правок — **gap-анализ построчно против кода** (grep по признакам функции, а не по названию раздела), и только потом реализация. Иначе переписываешь готовое и ломаешь то, что клиент уже принял. Побочный эффект правильный: список работ сокращается втрое, и его можно честно показать заказчику.

### Транспайл-онли сборка не видит ТРЁХ классов ошибок, которые ловятся первым же кликом
- За одну сессию поймано три «зелёная сборка → белый экран»: (1) правка внесена в область видимости внешнего компонента, а разметку рисует вложенный (`filters is not defined` — шапку отчёта рисует `ReportBandTable`, а не `DepartmentReportView`); (2) добавлен хук в компонент, где не было `import * as React` (`React is not defined`); (3) расширен строковый юнион, а `switch` в другом файле остался неисчерпывающим и молча вернул `undefined` (`Cannot read properties of undefined (reading 'map')` в `notificationLinksFor`).
- Все три — следствие отсутствия `tsc` в проекте (`vite build` = esbuild). Практический вывод: **после каждого шага открывать экран в браузере**, а не копить правки до конца задачи; и при расширении юниона грепать по имени типа, чтобы найти все `switch`/`Record`, которые обязаны его исчерпывать.

### Расширяя перечень типов, правьте ВСЕ производные таблицы, а не только ту, что на виду
- Новый `NotificationType` требует правок в пяти местах (с 25.09 аудитории по умолчанию `notificationAudienceFor` нет — кто получает, решает настройка ролей, а явный `visibleTo` ставится только событию, чей адресат зависит от самого события: этап, личное обращение): `NOTIFICATION_TYPE_META` (label/тинт), `notificationLinksFor` (дип-линки), `TYPE_ICONS` в карточке, наборы в `DEFAULT_ROLE_CONFIG` и список валидации сохранённого конфига (иначе новый тип отфильтруется при чтении из localStorage и «пропадёт» после первого сохранения настроек).
- `Record<Type, …>` компилятор бы проверил, а `switch` с возвратом — нет. Там, где важна исчерпываемость и нет tsc, предпочитайте `Record`: он хотя бы падает на глазах при первом рендере, а не отдаёт `undefined`.

### Фильтр в заголовке столбца: меняйте точку входа, а не логику
- Требование «фильтры как в Excel» выглядит как переделка фильтрации, но ею не является: логика (`applyReportFilters`, форма `ColumnFilter`, счётчик активных) остаётся ровно та же, меняется только **точка входа** — воронка в заголовке вместо панели. Общий `ColumnFilterBody` рендерит контрол в обоих контекстах, поэтому поведение не может разойтись; прежняя панель сохраняется как вход для мобильных, где в шапке места физически нет.
- Единственное осмысленное расхождение — enum-колонка: в панели это компактный поповер (иначе 30 колонок дадут бесконечный список), в воронке — сразу чекбокс-список. Такие исключения стоит проговаривать в комментарии, иначе следующий разработчик «унифицирует» их обратно.

### Сид, который «иллюстрирует» правило, обязан ему подчиняться — иначе живая арифметика превращает его в баг
- Даты в `PLAN_APPROVALS` писались как повествование («тут в срок, тут просрочка»), а не считались по правилу: **6 расхождений в 4 записях из 6**. Пока обе стороны читались из сида, это было незаметно. Но `/audit` (E-3) правило уже считал — и показывал «Просрочено» там, где план рисовал «В срок»; а Волна 4 добавила пересчёт в самом плане, поэтому строка меняла статус **просто оттого, что по ней появился живой цикл**. Клиенту это читается как «система сама себе противоречит».
- Правило: вводя деривацию поверх сида, прогоните **весь сид через ту же формулу** и почините расхождения до демонстрации. Считать глазами бесполезно (`addWorkingDays` сохраняет время суток, `workingDaysBetween` считает пересечённые будни, а `getOverdueDays` — целые сутки): 10-строчный скрипт, повторяющий хелперы, ловит всё за минуту и заодно проверяет порядок событий (ОД не раньше КД).
- Вторая находка того же прогона: **две подсистемы могут честно считать один срок в разных единицах** — план печатает рабочие дни, `/audit` календарные. Совпадения величин добиться нельзя (3 раб. дн. через выходные = 4 кал. дн.), поэтому цель формулируется как «совпадает **результат** (в срок / просрочено)», а расхождение величин фиксируется явно и планируется к выравниванию. Побочно это даёт правило для авторов сидов: перелёт дедлайна меньше суток — худший выбор для демо-данных, потому что именно там формулы расходятся в самом результате.

### Список «Known Issues» устаревает молча — его надо сверять, а не только дополнять
- `doc_sync` этой волны нашёл в `docs/AI_CONTEXT.md` три факта, неверных **не из-за текущей работы**: «План акций … no persistence» (персистентность появилась 17.07 в «Группе A» T12), PR-2026-007 как пример просрочки (в «7-й части» §1.3 его перевели в пример отправки **в срок**), «only E-4 remains» (E-4 закрыт 07.07). Все три пережили по несколько раундов документирования, потому что каждый раз сверху добавлялась новая строка, а старые пункты не перечитывались. Правило: в `doc_sync` списки ограничений и «осталось сделать» проверяются **против кода**, а не переносятся как есть; пункт, который что-то отрицает («нет персистентности», «не поддерживается»), — первый кандидат на проверку.

---

## 2026-08-05 — 10-я часть, Волна 5 (блок 5C «Аудит-лог и контроль сроков»)

### Radix-табы не переключаются ни `element.click()`, ни синтетическим PointerEvent — весь табличный QA обязан идти настоящими кликами
- Первый прогон матрицы «роль × вкладка» дал одинаковые 27 строк на всех четырёх вкладках. Это был не баг экрана: JS-клик по `[role="tab"]` не переключает Radix-таб, поэтому замерялась одна и та же вкладка четыре раза. Синтетический `dispatchEvent(new PointerEvent('pointerdown'/'pointerup'))` тоже не помог — Radix отсеивает недоверенные события. Работает только `browser_click`.
- Признак подделки в данных: цифры совпадают **точно** на всех вкладках, и `[role="tab"][data-state="active"]` не меняется после «клика». Проверять активную вкладку в том же замере — дешёвая страховка от целого класса ложных «проверено».
- Следствие для больших матриц: чистые деривации выгоднее проверять **прямым импортом модулей через dev-сервер** (`await import('/src/lib/x.ts')` в `browser_evaluate`) — 9 ролей × 4 вкладки считаются одним вызовом и без кликов, а кликами затем подтверждается только проводка UI. Так пойманы три дефекта, которые перебор кликами не показал бы вовсе.

### Ограничение «по автору» не выполняет требование «только свои объекты» — нужно ограничение по самому объекту
- Скоуп КМ фильтровал записи, у которых автор/ответственный — КМ, и пропускал ВСЁ остальное. В результате КМ видел решения КД и старшего КМ по чужим акциям, то есть «только свои промо» не выполнялось (9 акций из 9). Правило: ограничение по личности закрывает лишь записи этой роли; чтобы получить «свои объекты», нужен второй предикат по самому объекту (здесь — `campaignIds` из состава участников акции). Проверять надо не «сузилось ли», а **какие сущности остались**: `new Set(rows.map(r => r.campaignId)).size` против общего числа.
- Тот же класс ошибки повторился на третьей поверхности: ростер рейтинга строился из справочника и скоупу не подчинялся, поэтому КМ видел строки чужих КМ с «Нет данных». Список, который строится НЕ из отфильтрованных данных, а из справочника, — всегда отдельная дыра в гейтинге.

### Вводя общий скоуп вместо частных гейтов, проверьте, что новый скоуп не ШИРЕ прежнего частного
- До 5C вкладка «Сроки по плану» отдавала рядовому КМ пояснение вместо строк. Обобщённый скоуп включал объект «план» в набор КМ — и КМ увидел бы все 27 плановых строк. Замена набора частных гейтов одним общим по умолчанию **расширяет** права: каждый снятый частный гейт нужно явно перенести в модель скоупа, а после — сравнить «было/стало» по каждой роли, а не только убедиться, что новый гейт работает.

### «Ожидается» с ненулевой просрочкой — это противоречие в глазах клиента, а не нейтральное состояние
- `resolve()` возвращал «Ожидается» всегда, когда факта нет, при этом соседняя колонка печатала «+N дн.». Для пользователя это «система сама себе противоречит». Правило то же, что в E-3 про метрики, но применённое к отображаемому результату: у любого срока три исхода — выполнено вовремя, выполнено поздно, **не выполнено и уже поздно**; третий обязан называться просрочкой везде, где он виден, иначе колонка и рейтинг рассказывают разное.

### Прежде чем строить «недостающее» поле, посмотрите, какое существующее уже несёт этот факт
- Для контрольной точки «повторная отправка» напрашивались два поля (`returnedAt` + `resubmittedAt`). Но повторно отправленный набор перезаписывает `submittedAt` — значит текущая отправка И ЕСТЬ повторная, и хватает одного поля «когда возвращали». Модель осталась на одно поле легче, а срок считается от возврата. Перед расширением типа стоит спросить: не является ли «новый» факт уже записанным в существующее поле в другой момент времени.

### `overflow-hidden` на предке ломает `sticky bottom-0`, `overflow-clip` — нет
- Продолжение урока Волны 4. Закреплённая снизу полоса, вложенная в карточку с `overflow-hidden`, липнет к низу КАРТОЧКИ (hidden создаёт контейнер прокрутки) — то есть остаётся ниже вьюпорта, ровно там, где и была проблема. `overflow-clip` обрезает скругления, но контейнером прокрутки не становится, и sticky разрешается относительно скроллпорта страницы. Проверка — не на глаз: `track.getBoundingClientRect().bottom` против `window.innerHeight`.
- И обратная сторона: жалобу «нижнего скролла не видно» нельзя закрывать рассуждением «контейнер ограничен вьюпортом, значит полоса видна». Ограничение `max-h-[calc(100vh-N)]` задаёт высоту, но не положение: если бокс начинается низко на странице, его нижняя кромка (а с ней и полоса) уходит за экран. Замерять `getBoundingClientRect().bottom`, а не читать класс.

---

## 2026-08-05 — 10-я часть, Волна 5 (блок 5D «Пользователи, роли, замещение»)

### Пустая коллекция как «данных нет» — тихий откат к legacy-полю, и гвард считает не то состояние
- `assignmentsOf` трактует пустой `roleAssignments` как «реестра ещё нет» и восстанавливает роли из старых `role`/`roles`. Это правильный фолбэк для миграции, но он же означает, что запись ПУСТОГО реестра отменяет только что сделанное снятие роли: `canRevokeAdmin` симулировал отзыв, получал пользователя с `roleAssignments: []`, а `permanentRolesOf` возвращал ему «Администратор» из legacy-поля — гвард отвечал по состоянию, которое никогда не наступит.
- Правило: если у структуры есть фолбэк «пусто = не задано», то пустое значение обязано быть **недостижимым при записи**. Лечится инвариантом на границе (здесь: постоянных ролей всегда ≥1 и ровно одна основная), а не проверками у каждого вызывающего. Проверять надо не «сузилось ли», а прогонять сам сценарий снятия до конца: `usableAdminCount()` до и после должен упасть.

### Право, выданное на срок, не должно учитываться там, где считается устойчивость системы
- Соблазн был читать один набор ролей везде («так же честнее»). Но пул администраторов, гарантирующий вход в систему, обязан считаться по ПОСТОЯННЫМ ролям: временный админ на две недели формально закрывает требование «не менее двух», а в день истечения окна система остаётся без администраторов — причём гвард в этот момент отчитается, что всё в порядке. Тот же принцип отдельно применился к пункту меню «Отозвать права администратора»: по временной роли отзывать нечего.
- Обобщение: вводя срочные права, пройдитесь по всем местам, где число носителей роли что-то ГАРАНТИРУЕТ (пул админов, наличие согласующего, кворум), и явно решите, какой из двух наборов там корректен. Это не то же самое, что «где показывать роль».

### Роль, которой нет в перечислении, нельзя «примапить» на соседнюю — проецируйте её в отображение
- «Уполномоченное лицо КД» отсутствует в `PROMO_ROLES`. Спроецировать замещение на «Коммерческий директор» внутри `activeRolesOf` выглядело элегантно и сломало бы гейтинг молча: заместитель мгновенно получил бы всё, что гейтится по этой роли (навигация, `permissions.ts`, матрица прав), тогда как ТЗ ограничивает замещение этапом согласования. Решение — отдельная функция `substitutionBadgeFor`, дающая только чип; гейтинг остаётся за прежним `canActAsKd`, и S3 не тронут ни строкой.
- Проверять надо не «появился ли чип», а обратное утверждение: под заместителем `activeRolesOf` НЕ содержит «Коммерческий директор», админ-скоуп пуст, пункт навигации отсутствует, экран отдаёт заглушку.

### Сравнение «до/после» на сидах от `Date.now()` требует контрольного замера, иначе припишете эффект не тому
- Проверка «назначение замещения не сдвигает сроки согласования» на первом прогоне дала расхождение — и это выглядело как настоящий баг. Причина: `buildReviewItems` строит `submittedAt` относительно текущего момента, между двумя вызовами прошла 1 мс, и величина просрочки перешагнула границу суток (3 → 2).
- Правило: прежде чем объяснять расхождение проверяемым действием, сделайте **контрольный замер без него**. И считайте от ЯВНОЙ точки отсчёта (`itemSla(item, REF)`), а не от `new Date()` внутри вызова — тогда «до» и «после» сравнимы побайтно.

---

## 2026-08-05 — 10-я часть, Волна 6 (распределение промо по КМ / дням / категориям)

### Классификация действия в аудите — часть требования, а не косметика
- Распределение писалось существующим типом «изменение», чтобы не расширять `AuditActionType` (осознанная экономия, урок E-4 про неисчерпаемые `Record`). Но 5C отнесла «изменение» к НЕ-ключевым действиям, и коммерческий директор **не видел собственное распределение** в режиме «Ключевые действия», который у него включён по умолчанию. Для пункта, чей смысл — прозрачность («кто и как распределил акцию»), это провал требования, а не мелочь отображения.
- Правило: выбирая существующий тип действия вместо нового, проверьте не только «пишется ли событие», но и **кто и в каком режиме его увидит**. Дешёвая проверка — открыть лог под ролью, которая это действие совершает, в её дефолтном режиме.

### «Пусто» и «записи нет» — разные состояния, и оба нужны
- В сторе распределения пустой массив означает «распределения нет» (КД сознательно убрал все строки), а отсутствие ключа — «действует сид». Схлопнуть их в одно значение соблазнительно, но тогда «Очистить распределение» стало бы неотличимо от «вернуть как было», а для клиента это разные операции: первая переводит акцию на общую логику, вторая отменяет правку.
- Правило для мок-сторов поверх сидов: определите обе операции явно (`set(id, [])` и `clear(id)`) и проверьте каждую отдельным сценарием — иначе разница всплывёт у пользователя, а не в тесте.

### Один и тот же ask в трекере может лежать двумя строками с разными формулировками
- Строки 73 (29.07, 7 подпунктов) и 74 (30.07, 9 подпунктов) описывают одно требование: вторая — уточнённая редакция первой. Считать их двумя разными задачами значит удвоить объём и получить два несогласованных решения.
- Признак: совпадают объект («Распределение по категориям»), актор (КД) и точка входа («План акций»), различаются только детализация и порядок пунктов. Перед планированием стоит свести такие строки в одну таблицу подпунктов и работать по ней.

---

## 2026-08-06 — «11-я часть»: невидимая валидация, сид vs живой журнал, экспорт = проекция экрана

### «Я всё заполнила» = валидация видна только валидатору
- Клиентское «не получается отправить, хотя всё заполнено» оказалось правдой с обеих сторон: поле действительно пустое, но увидеть это нечем. Маркер «не заполнено» стоял в 30-й колонке из 38 — на ~5500px при видимых ~600px; `*` в шапке не было; подсказка «N строк: не заполнены обязательные поля» не называла ни строк, ни полей.
- Правило: сводка валидации обязана **называть строки и поля и вести к ним** (список → клик → карточка строки, где поля собраны вертикально). Голый счётчик в широкой таблице — это не подсказка, а стена.
- Смежное: gating «всё или ничего» превращает одну незаполненную строку (даже чужую) в блокировку всей отправки. Выбор строк должен задавать и набор отправки, и набор валидации.

### Мёртвая ветка валидации живёт с переименования id
- `isRequiredFor` проверял id `giftNomenclature`/`giftStock`, исчезнувшие при переработке подарков 01.07 — звёздочка обязательности не рисовалась больше месяца, и никто не заметил, потому что ветка «тихо возвращала false». При переименовании полей — grep по всем **строковым** ссылкам на старые id, они не ловятся ни билдом, ни типами.
- Подписи обязательных полей брать из словаря колонок (`requiredFieldLabel` над `COLUMNS`), а не дублировать строками — тогда расходиться нечему.

### Сид-состояния обязаны материализоваться в живой журнал при первом живом событии
- Деривация «открытый цикл ИЛИ сид» коварна на переходе: первое живое решение ОД по строке, чьё согласование КД существует только в сиде, создало бы цикл без КД — и таблица «забыла» бы согласование КД. `ensureOpenCycle` теперь копирует сид-решения директоров в материализуемый цикл №1 (семантика `approvedStages`).
- Обобщение: если живой слой перекрывает сид ЦЕЛИКОМ (а не по-полям), при первой живой записи в него нужно перенести все сид-факты, которые обязаны пережить переход.

### Гейт «этап по статусу всего объекта» прячет доступ от параллельного актора
- ОД не видел действий, потому что этап выводился из агрегатного `planStatus`, а не из роли: пока КД не согласует ВСЕ строки, план «На согл. с КД» и ОД бездействует — хотя таблица уже показывает строки «Согласовано КД». Клиент дважды репортил это как «нет доступа» — и был прав, а первая диагностика («не воспроизводится») ошиблась, потому что смотрела на текущую механику, а не на ожидание клиента.
- Правило: если UI показывает пер-строчные состояния этапов, права действия должны выводиться из тех же пер-строчных состояний. И: повторный репорт клиента после «не воспроизводится» — сигнал сменить гипотезу, а не повторить проверку.

### Экспорт — проекция экрана: тот же порядок, те же агрегаты
- Клиентское «скачанные данные не все корректные» распалось на два расхождения с экраном: порядок строк (сид vs сортировка по периоду) и «Статус строки» («Отправлено» при обоих согласованных этапах). Клиент сверяет файл с таблицей построчно — экспорт обязан читать ту же сортировку и ту же композицию статуса, что и рендер (в идеале — буквально те же функции).
- Два разных списка одного факта на экране (тост считал пробелы дат по отправленным строкам, баннер — по всему плану) читаются как баг, даже если каждый по-своему «прав». Один факт — один источник.

### Проп добавлен в тип, но не в деструктуризацию — падение только в рантайме
- `SubmitButton({ canSubmit, reason, onClick }: { …; count: number; … })` + `count` в JSX тела = `ReferenceError`, экран падает в ErrorBoundary. esbuild-билд (транспайл без tsc) это пропускает; поймано только открытием страницы. Конкретизация давнего урока «vite build не тайпчекает»: после ЛЮБОЙ правки сигнатуры компонента — открыть экран, а не только собрать.

### Файл от менеджера в Telegram новее файла на Desktop
- Статусы разработчиков были заполнены в копии трекера от 04.08, тогда как менеджер уже прислал версию «до_06» (06.08) с новыми колонками и комментариями. Перед работой с клиентскими файлами — сверить даты всех копий (`Desktop` vs `Downloads\Telegram Desktop`); структура колонок между версиями меняется (13-я колонка стала 4-м комментарием Б/А).
- Запись в xlsx менеджера — через Excel COM (сохраняет форматирование), не через SheetJS CE (теряет стили); .ps1 с кириллицей для PowerShell 5.1 обязан иметь UTF-8 BOM, иначе парсер читает ANSI и падает на «сломанных» кавычках.

---

## 2026-08-06/07 — Promo «11-я часть», вторая порция (форма распределения по визуалу PM + комментарий 06.08)

### Новая версия клиентского файла — сначала пофайловый дифф, потом gap-анализ
- «Добавили новые комментарии, посмотри» на 76-строчном трекере — это НЕ повод перечитывать все 76 строк: дифф двух xlsx по ячейкам (SheetJS `sheet_to_json` + сравнение) показал, что новая ячейка ровно одна (R46, колонка M). Всё остальное «новое» на глаз оказалось бы уже сделанным.
- Макет от PM проверять на внутренние противоречия ДО реализации: на визуале «Среда 15.12.2026» (среда = 16.12), распределение открыто для строки «Черновик» (противоречит их же тексту «на этапе согласования») и КД числится «Ответственным КМ». Ошибки макета фиксируются в ответе и НЕ переносятся в код; дата+день недели рендерятся из одного значения — тогда класс ошибок «подпись разошлась с датой» невозможен по построению.

### У SheetContent из @texnomart/ui НЕТ собственного паддинга
- shadcn-Sheet здесь даёт паддинг только в `SheetHeader/Footer` (`p-4`); тело, положенное прямо в `SheetContent`, прилипает к краям — так ушли в прод две панели («Детали изменений» и панель проверяющего), и клиент это заметил (Блок 4.1). Приём `-mx-6 … px-6` на sticky-футере молча предполагал `p-6`-родителя, которого не было.
- Правило для новых правых панелей: явные `px-6` на шапке И теле (или общий контейнер с паддингом), а отрицательные поля футера — только зеркалом реально существующего паддинга.

### Полное (необрезанное) имя в Pattern F: высоту строки считать канвой ДО отрисовки
- «Полное наименование без обрезки» в split-pane таблице нельзя сделать простым снятием clamp: обе панели обязаны получить одинаковую высоту строки до рендера. Решение — `canvas.measureText` той же гарнитурой (жадный перенос по словам) с **консервативной** шириной: недооценка ширины даёт лишнюю высоту (безопасно), переоценка — клиппинг (нельзя). Слово длиннее строки учитывать отдельно (`break-words` его перенесёт).
- Результат идёт в тот же `lineHeightPx`, что и подстроки подарков — `max()` обоих требований, инвариант панелей сохранён.

### Кампанийный предикат, выведенный из строчной логики, обязан зеркалить её ветвление ЦЕЛИКОМ
- Наивный `isCampaignDraft = status==="Черновик" || (!planned && !firstSendDone)` скопировал только черновиковые ветки `lineDisplayStatus` и спрятал от КД согласованную акцию и акцию на его же решении: в строчной функции эти ветки недостижимы, потому что `switch` по статусу выходит раньше. Копируешь ветку — копируй и ранние выходы перед ней.
- Ловится за минуту сравнением наборов по ролям в браузере: список «26-N» под Админом минус под КД должен совпасть с ожидаемым множеством черновиков.

### «Пустой список» может быть валидным сохранением — не блокируй submit на length===0 не сверив семантику стора
- В сторе распределения пустой массив («распределения нет») и отсутствие записи («действует сид») — РАЗНЫЕ состояния; «Сохранить» при нуле строк — законный способ убрать сидовое распределение. Рефлекторный `disabled={rows.length===0}` на кнопке сохранения молча отрезал бы этот сценарий (поймано самопроверкой перед коммитом).

### «Действия из окна деталей» ≠ одинаковые действия для всех строк — уважай модель этапа
- Клиентское «Согласовать/Отклонить перенести в окно деталей» для первичного S3-потока упирается в модель: пер-строчного approve там нет (набор согласуется целиком). Правильный перенос — не изобретать новое состояние, а разветвить футер панели: повторные строки получают обе кнопки (+подтверждение), первичные — только «Отклонить строку» (существующий обязательный комментарий) и пояснение, как согласуется набор. Требование UX удовлетворено, инварианты контура не тронуты.

---

## 2026-08-17/19 — Client Broker (новый саб-проект, Alif-флоу, перенос мока в Figma)

### Playwright MCP: общий профиль браузера лочится между конкурентными агент-сессиями
- Каждый субагент-имплементер, пытавшийся открыть браузер, пока живы соседние сессии, получал «browser profile locked». Работает надёжно одно: весь настоящий click-through QA — ОДНОЙ сольной задачей в конце волны, имплементерам — build + чтение кода.
- Dev-серверы глушить только по PID (`netstat -ano | findstr :<port>` → `taskkill /PID <pid> /F`). Один агент сделал `taskkill /F /IM node.exe /T` и убил ВСЕ node-процессы машины, включая чужие dev-серверы.

### Git Bash (MSYS) искажает env-значения, похожие на пути: `BASE_PATH=/Texnomart/broker/` превращается в `C:/Program Files/Git/Texnomart/broker/`
- Проверять subpath-сборки (`BASE_PATH=… pnpm build:…`) на этой машине надо из PowerShell (`$env:BASE_PATH='…'; corepack pnpm build:…`), иначе в `dist/index.html` окажутся пути с `/Program Files/Git/…` и диагностика уведёт не туда.

### Figma capture (html-to-design) из живого приложения: await зависает — fire-and-poll
- Схема переноса мока в Figma: инжект `mcp.figma.com/mcp/html-to-design/capture.js` в живую вкладку через `browser_evaluate`, довести SPA до нужного состояния, `window.figma.captureForDesign({captureId, endpoint, selector:'body'})`, потом поллить `generate_figma_design` с этим captureId. В SPA скрипт переживает внутренние навигации (но не перезагрузку) — 14 состояний сняты одной вкладкой.
- `await captureForDesign(...)` внутри `browser_evaluate` висит бесконечно (капча уже завершилась на сервере, промис — нет): вызывать БЕЗ await («fired») и подтверждать завершение поллингом. Каждый captureId одноразовый; переходные состояния (скелетон лимита ~6 с) ловить, форся состояние через sessionStorage перед перезагрузкой, а не гонкой со временем.

### Локальный стейт как источник данных для попапа — источник тихой порчи при reopen
- Чип срока жил в локальном useState карточки банка и сбрасывался на дефолт при ремаунте; безусловный `selectAlif(tenor)` на каждом «Оформить» переписывал уже подтверждённый/оформленный срок при повторном открытии (после перезагрузки договор показывал 6 мес. вместо выбранных 12). Правило: всё, что фиксируется этапом флоу, замораживается в сторе гейтом (`if (!offerConfirmed) selectAlif(...)`), а не читается заново из UI-стейта.

## 2026-08-19 — Client Broker: «уже реализовано» ≠ «достижимо на демо»

**Контекст.** PM прислал чек-лист из 5 пунктов по Alif («проверить/добавить»). Сверка с кодом: 4 из 5 были написаны ещё 18.08. Но клиент/PM их не видел — и был прав.

**Урок 1. Ветка обработки ошибки, которую нечем вызвать, — мёртвый код.** `validatePhoto` проверяла MIME фото, экран ошибки с «Переснять» был полностью написан. Только вот `capture()` рисует кадр в canvas и отдаёт `toBlob(..., "image/jpeg")`, демо-фото приводится к `image/jpeg` принудительно, а проверку размера сняли по просьбе PM 18.08 — то есть после того снятия функция **не могла вернуть ошибку никогда**. Экран существовал в репозитории и не существовал в продукте. При ревью «реализовано ли X» смотреть не на наличие кода, а на то, есть ли вход в него: какой ввод, какое состояние, какой ответ сервиса его включает. Если ответа нет — пункт не закрыт.

**Урок 2. В моке исход задаётся руками, и это нормально — но помечается как временное.** У карты для этого есть суффиксы `0000`/`9999`, у OTP — `OTP_FAIL_CODE`. У фото поля ввода нет, поэтому сделан явный чип-переключатель «Демо-сценарий: Успех · Некорректное фото · Отказ MyID». Такие аффордансы обязаны быть подписаны как демо в коде и в доках (`MYID_DEMO_SCENARIOS` + раздел в `Broker/CLAUDE.md`), иначе они уезжают в прод.

**Урок 3. Статус, живущий 600 мс, — это отсутствующий статус.** `holdStatus` имел все четыре значения ещё 18.08, но `confirmed` был виден только в окне задержки перехода фазы, а `cancelled` вообще схлопывался в `none` при отрисовке (`HoldPhase`) — при том, что экран текстом обещал «до завершения оформления холд можно отменить». Просьба «покажите статусы» была не про модель данных, а про то, что модель нигде не выведена.

**Урок 4. Разделять «отменить операцию» и «выйти из процесса».** Старый `holdCancel` делал и то и другое: снимал холд, сбрасывал `offerConfirmed` и закрывал попап. Из-за этого отменить холд «и остаться» было нельзя. Разнесено на `holdCancel()` (только статус) и `cancelOffer()` (сброс оферты + закрытие + разморозка срока). Побочно выяснилось, что деривация фаз должна ставить «предоплата не удержана» **выше** «доп. данные введены» — иначе отмена холда с шага OTP оставляла оператора на OTP при разблокированных деньгах. Порядок проверок в `checkoutPhaseOf` читается как «что сейчас блокирует прогресс», а не «как далеко зашли».

**Урок 5. Проверка типов в этом репозитории — только чтением и браузером.** TypeScript в зависимостях нет вообще (`Broker/package.json` — ни `typescript`, ни `tsconfig.json`), `vite build` транспилирует через esbuild без проверки типов. Зелёный билд не означает, что типы сходятся.

**Приём QA.** Ждать 120 с истечения OTP в браузере незачем: временно уменьшить `OTP_TTL_SECONDS` до 5, дождаться HMR, снять состояние, вернуть константу. Дешевле и надёжнее, чем «поверить, что таймер сработает».

**Figma-захват: `generate_figma_design` без `nodeId` НЕ создаёт новую страницу.** Описание инструмента обещает «If not provided, a new page is created in the file», но по факту все 11 кадров легли на текущую страницу файла («Client Broker»), в ряд справа от существующего контента (x от 55 566 с шагом 1480). Разные префиксы node-id (456:…, 457:… — новая сессия правки на кадр) на страницу не указывают: проверять принадлежность надо подъёмом по `parent` до `PAGE`. Приём: снять всё, потом одним `use_figma` создать страницу, `appendChild` кадры, переименовать и разложить сеткой — перенос дешевле, чем попытка угадать `nodeId` целевой страницы заранее.

**`figma.root.children` показывает страницы, но `page.children.length` у незагруженных страниц врёт (0).** Страницы грузятся лениво; достоверен только счётчик текущей страницы. Для проверки «куда легло» — `getNodeByIdAsync` + подъём по родителям, а не обход страниц.

**Профиль Playwright.** Подтвердилось снова: профиль `mcp-chrome-*` остаётся залоченным брошенным процессом от прошлой сессии. Лечится `Stop-Process` по корневому PID (найти через `Get-CimInstance Win32_Process` с фильтром по `CommandLine`), после чего MCP поднимает браузер штатно.

---

## 2026-08-21 — Client Broker: третья итерация ветки Alif (ТЗ на основе API)

### Мёртвый код, найденный пять раз за одну итерацию — недостаточно спросить «есть ли код», нужно спросить «что до него доводит»

Урок 19.08 («ветка обработки ошибки, которую нечем вызвать, — мёртвый код») подтвердился не один раз, а **пять**, за одну и ту же итерацию, каждый раз ловился только ревью (в двух случаях — только чтением кода ревьюером, до всякого браузера) и ни разу — ни билдом, ни имплементором, который этот код только что написал. Каждый случай — свой механизм недостижимости, поэтому «код есть» проверять пятью разными способами:

1. **Экран, до которого деривация никогда не доводит.** `ApplicationPhase` изначально содержала собственную ветку `REJECTED`/`CANCELLED` — синтаксически корректную, отрисовывающую ровно то, что нужно. Только вот `checkoutPhaseOf` не смотрит на `application.status`, а деривация фазы `hold` идёт сразу после `application` без остановки на статусе, — то есть заявка, ставшая `REJECTED` или `CANCELLED`, немедленно уводила оператора на фазу холда, и веткам внутри `ApplicationPhase` было физически негде отрисоваться. Найдено pre-flight сканом плана (не имплементором, не ревьюером кода — предварительным анализом деривации до старта задач), исправлено переносом обоих терминальных экранов на уровень `AlifCheckoutDialog`, **выше** переключателя фаз.
2. **Плашка на фазах, которые теперь идут раньше того состояния, о котором она сообщает.** `HoldStatusBar` («Предоплата удержана») изначально висела на `details` и `application` — унаследовано от второй итерации, где холд шёл ДО доп. данных. В новом порядке холд идёт ПОСЛЕ обеих: деривация пускает на `details` только при пустых `relations`, на `application` — только при отсутствующей заявке, и ни в одном из этих состояний холд не может быть подтверждён. Плашка была в репозитории и не существовала в продукте на этих двух экранах. Поймано до кода — при планировании следующего шага (Ruling R9), проверено чтением: `HoldStatusBar` реально мон­тируется только на `otp`.
3. **Кнопка отмены, чьё единственное место монтирования гарантирует, что её же гварда никогда не будет `true`.** Первая версия правила «отменить холд можно только пока заявка новая» оставила саму кнопку «Отменить холд» **только** на `HoldStatusBar` — а `HoldStatusBar` монтируется только на `otp`, куда деривация пускает исключительно когда заявка уже НЕ `NEW`. Значит `canCancelHold` на этом экране всегда `false`, а исполнитель защищал находку доводом «так и задумано». Опровергнуто чтением `HoldPhase` целиком: там не было ни одной точки вызова `holdCancel`. Это третий случай того же класса дефекта в одном плане — контроллер уже специально проверял это грепом перед тем, как отдавать ревьюеру. Исправлено переносом самой кнопки в `HoldPhase` (ветка `confirmed` — единственное состояние, где заявка гарантированно `NEW`).
4. **Демо-опция, называющая статус, которого приложение никогда не присваивает так.** Демо-селектор экрана 4 первой версии предлагал «Одобрено сразу» — вариант, ведущий заявку прямиком в `APPROVED`, минуя `REVIEWING`. Но в реальном коде `APPROVED` присваивается **ровно в одном месте**: таймер рассмотрения в `AlifCheckoutDialog`, и всегда как переход из `REVIEWING`. Прямого пути в `APPROVED` не существует нигде. Демо-опция называла поведение, которого в приложении нет — и попутно тихо ломала три инварианта, помеченных «менять нельзя» (`checkoutPhaseOf`, `canCancelHold`, `submitForReview` — все считают, что заявка стартует `NEW`). Исправлено не поведением, а подписью: вариант убран, оставшийся неотказной переименован в «Заявка принята».
5. **Состояние загрузки, размонтированное в том же коммите React, где оно было выставлено.** `ApplicationPhase` вызывала `setSubmitting(true)` и тут же, синхронно в том же обработчике, `createApplication(...)`. React батчит оба вызова в один коммит; а создание заявки меняет `state.application`, из-за чего деривация фазы немедленно уводит с `application` — то есть спиннер «Создаём заявку…» не может отрисоваться никогда: компонент размонтируется в том самом рендере, где должен был появиться. Требование ТЗ «состояние загрузки на каждом экране» формально было реализовано и фактически не выполнялось. Исправлено переносом `createApplication()` в отдельный макротаск (`setTimeout`, `APPLICATION_SUBMIT_DELAY_MS = 900`), чтобы состояние `submitting` получило собственный кадр отрисовки до того, как фаза сменится; тот же приём и та же причина применены к отложенной привязке карты (`CardAttachPhase`).

Общий вывод: «реализовано ли X» не проверяется наличием файла/ветки/JSX — проверяется тем, есть ли путь из состояния приложения, реально доступного оператору, до этой ветки. Для фазовых машин это означает: после любого изменения порядка/условий в `checkoutPhaseOf` — перепроверять каждую компоненту, которая на что-то реагирует условно, а не просто читать диф файла, где она объявлена.

### Один и тот же выбор в двух местах — тот же баг находит новую форму дважды

Урок 18.08 («локальный стейт как источник данных для попапа — источник тихой порчи при reopen») сработал снова, дважды, в этой же итерации — под разными масками, но с одной и той же причиной: **если одно и то же значение можно записать из двух разных компонентов, они рано или поздно разойдутся**, потому что запись в одном не обязана быть видна другому синхронно.

- **Срок рассрочки.** Ровно причина, по которой §2.2 спеки убрала чипы срока с `BankCard` целиком, а не просто дублировала их на экране 1: держать выбор в двух местах — ровно та конструкция, что 18.08 породила баг «договор на 6 мес. вместо выбранных 12» (локальный `useState` карточки перезаписывал подтверждённый срок при повторном открытии). Урок применён превентивно — выбор условия теперь существует **только** на экране 1 попапа (`OfferPhase.selectPlan`), `BankCard` вообще не знает о сроке.
- **Демо-переключатель «совпадение телефона».** Тот же класс бага возник заново, уже в новом коде: `DemoScenarioBar` и `CardAttachPhase` изначально держали каждый свою копию `phoneMatch`. `sessionStorage` не спасает — запись в одном компоненте не шлёт событие `storage` в той же вкладке, так что вторая копия узнаёт об изменении только после следующего полного цикла чтения (в этом случае — никогда, до следующего маунта). Результат: переключение пилюли в баре не перерисовывало фазу, а в `attachAlifCard()` уходило устаревшее значение — то есть ломалось ровно то поведение, ради которого экран и делался. Исправлено подъёмом значения в общего хозяина обоих компонентов (`AlifCheckoutDialog`, который и так рендерит и бар, и фазу) и передачей пропом в обе стороны; `sessionStorage` остался только слоем персистентности между сессиями, не каналом синхронизации внутри одной.

Правило одно и то же в обоих случаях: если значение управляет **выбором пользователя**, у него должен быть один компонент-владелец (или общий предок, если два потребителя равноправны) — второй источник правды всегда в итоге расходится с первым, вопрос только времени.

### Конфликт требований: чек-лист PM 19.08 vs. ТЗ 19.08 (по дате получения) — отмена холда

19.08 в этот репозиторий пришли **два** документа с одной датой и противоположными требованиями к одному и тому же месту UI. Чек-лист PM (реализован в тот же день, 19.08) сделал «отменить холд» доступным с любой поздней фазы попапа — тогда это было правильно для тогдашней пятифазной формы, где холд шёл ДО доп. данных и заявки как понятия ещё не существовало. `alif_ui_tz.md` (ТЗ по API Alif, получено тем же числом, но реализовано в этой, третьей итерации, 21.08) в экране 5 прямо пишет: отмена холда допускается **только пока заявка в статусе «Новый»** — после удержания заявка уходит на рассмотрение, и по модели самого API удержанная предоплата под рассматриваемой/одобренной заявкой — это уже не «просто передумал», а решение уровня заявки.

**Разрешение — в пользу ТЗ**, и вот почему: ТЗ написано по документации самого API и описывает реальное поведение бэкенда, тогда как чек-лист PM был решением уровня UI, принятым до того, как в модели вообще появилось понятие заявки. Возможность разблокировать деньги при этом **не исчезла — она переехала**: как только заявка выходит из `NEW`, отмена холда достигается через «Отменить заявку» (`CancelApplicationDialog`, доступна в `NEW`/`APPROVED`), которая отменяет заявку и снимает холд одним действием. Мёртвой зоны не образуется — в моке `REVIEWING` длится ~2,5 с и сам переходит в `APPROVED`, где «Отменить заявку» снова доступна. Две кнопки отмены на одном экране сознательно не сделаны: `HoldStatusBar` лишилась действия целиком, а не задублировала эффект «Отменить заявку».

Урок здесь не «чек-лист был неправ» — оба документа были правы **для формы попапа на момент своего исполнения**. Урок в том, что при столкновении двух источников требований с одной датой решает не порядок получения, а то, какой из них ближе к системе, чья модель на самом деле определяет поведение (здесь — API-бэкенд), и в задокументированном решении обязательно называть, куда делась старая возможность, а не просто что она исчезла.

### Демо-аффордансы — одна панель, а не россыпь чипов по фазам

Урок 19.08 уже показал, что реальных недостижимых исходов в этом моке будет много: отказ скоринга, `phone_match: false`, три статуса + шесть бизнес-ошибок заявки, истечение сессии — пять новых демо-точек за одну итерацию, поверх уже существующих чипов MyID. Рассыпать их чипами по каждой фазе (как было сделано для MyID 18–19.08) масштабируется плохо: пять фаз — пять мест для поиска при удалении, пять мест, которые могут разойтись по визуальному языку, и пять мест, которые надо не забыть убрать по одному при интеграции с реальным API.

Вместо этого демо-панель для ветки Alif третьей итерации собрана в **одном файле**, `checkout/DemoScenarioBar.tsx`, которую рендерит сам `AlifCheckoutDialog` тонкой полосой внизу попапа — она сама знает текущую фазу и показывает только релевантные ей переключатели. Одно место, один визуальный язык, **одна точка удаления** при интеграции с API — что явно помечено и в самом файле, и в `Broker/CLAUDE.md`. Чипы MyID сознательно не объединены с этой панелью (они завязаны на фазы съёмки камеры и живут вне попапа) — унификация двух разных демо-подсистем не была частью скоупа и не стоила бы риска ради чисто косметической консистентности.

Правило на будущее: если демо-триггеров на фичу больше одного-двух, они с самого начала собираются в отдельный компонент/файл, а не размазываются по местам, где их значение проверяется, — стоимость лишнего пропа/импорта многократно ниже стоимости пяти отдельных точек удаления, каждая из которых может быть забыта.

### Зелёная сборка здесь ничего не доказывает — это подтвердилось снова, теперь пятью примерами разом

Урок 19.08 уже фиксировал: в `Broker/` нет ни `typescript`, ни `tsconfig.json`, `vite build` транспилирует через esbuild и не проверяет типы. В этой итерации это было явно объявлено рулингом плана (R5) ещё на старте: «сборка зелёная перестаёт быть достаточной проверкой» — потому что удалённый экспорт, отсутствующее поле состояния, несоответствие `Record<CheckoutPhase, ...>` компилируются молча. И тем не менее пять случаев мёртвого кода выше (раздел «Мёртвый код...») — все они прошли через зелёный билд имплементора **до** того, как их поймало ревью чтением или, в одном случае, живой репродукцией в браузере. Билд не солгал — он просто не про это. Единственное, что реально ловит недостижимость ветки в этом репозитории: чтение деривации целиком (какие состояния реально приводят на этот компонент) и клик в браузере (что оператор реально может увидеть). Грепом по удалённым символам билд заменить можно — логической недостижимостью, объяснённой в предыдущем разделе, нет.

---

## 2026-09-11 — Promo: разбор клиентского трекера и 10 правок прототипа

### Ответ на комментарий клиента обязан называть стенд, иначе он не ответ

Вся путаница этого раунда выросла из одной пропущенной детали: ответы разработки от 01.09.2026 писались про **прод** — «подтверждено ответом бэкенда от 31.08», «сервер отдаёт только 500 последних записей», «включая промо 26-16 и 26-1» — а тестировщик проверяла **прототип**, где нет ни бэкенда, ни этих номеров. Её проверка 09.09 буквально это и зафиксировала: «26-16 его вообще нет, и 26-1 нет такого». Формально ответы были правдивы, практически — непроверяемы, и целый раунд ушёл на выяснение, что вообще имелось в виду.

В проекте с двумя стендами (мок на GitHub Pages и backend-сборка `promo.tm.uz`) у каждого утверждения «сделано» есть невидимый аргумент — где именно сделано. Если его не написать, читатель подставит свой стенд, и расхождение обнаружится только через круг переписки. Правило: в любом ответе по трекеру называть стенд явно, а номера сущностей приводить те, что существуют на стенде читателя.

### Один и тот же доступ, описанный в трёх местах — расходится ровно там, где не смотрят

«Сотрудник маркетинга» видел «Полный промо-календарь», хотя клиент просил закрыть доступ ещё 07.08. Причина оказалась не одна, а три слоя, каждый со своим мнением:
- `ACCESS_MATRIX` (экран «Матрица прав», наш «документ») — уже стоял `none`;
- `getFullCalendarAccess()` (enforcement-хелпер, который и решает) — возвращал `canView: true` ради одного поля «В рекламу (выбрано маркетингом)»;
- пункт меню в `shell-config.tsx` — вообще не имел `roles` и показывался всем.

Экранный гард на странице при этом существовал и честно работал — просто пропускал, потому что спрашивал у хелпера. Подправить только меню значило бы оставить доступ по прямой ссылке; подправить только хелпер — оставить расхождение с матрицей на следующий раз.

Сделано так: хелпер закрыт (возможность редактировать флаг не потерялась — она есть в отчётах через `getReportAccess().canEditMarketingFlag`), а меню переведено на новую `rolesWithAreaAccess()`, которая **читает список ролей из матрицы**. `permissions.ts` создавался ровно ради консолидации — но консолидация, которую никто не потребляет, остаётся документом, а не источником правды. Правило: если в проекте заведён единый реестр прав, гейтинг обязан его читать, иначе реестр расходится с поведением молча.

### Исправленный дефолт не виден тем, у кого есть сохранённый конфиг

`DEFAULT_ROLE_CONFIG` в `notification-settings-store.ts` был исправлен, но `getRoleConfig()` мержит сохранённое в localStorage **поверх** дефолтов — значит у любого, кто хоть раз открывал экран настроек уведомлений, осталась бы старая (неверная) раскладка, и тестировщик увидела бы ровно тот же дефект. Лечится версией ключа: `promo:notification-role-config` → `-v2`.

Обобщение: правка значения по умолчанию в сторе с персистентностью — это **две** правки. Вторая — решить, что делать с уже сохранёнными данными: версия ключа, миграция или явный сброс. Без неё правка работает только на чистом браузере, то есть у автора и ни у кого больше.

### Спрятанная ссылка бывает симптомом дефекта в данных, а не в гейте

Уведомление «Новая версия отчёта» по промо 26-1 не показывало кнопку «Открыть отчёт», хотя клиент просил её «везде». Соблазн — ослабить гейт в `notificationLinksFor`. Но гейт был прав: `/reports` перечисляет только `getSentCampaigns()`, у PR-2026-001 отчёт смежным отделам не отправлен, и ссылка вела бы в экран, который эту акцию показать не может. Настоящий дефект — в посеве: уведомление об отправке отчёта висело на акции, у которой отчёта нет. Исправлено переводом уведомления на PR-2026-003, где версия 4 действительно отправлена.

Правило: прежде чем ослаблять условие, проверить, не сообщает ли оно о противоречии в данных. Условие, которое «мешает», нередко единственное, что это противоречие ещё видит.

### `sticky` для закреплённой колонки: у shadcn `<Table>` уже есть свой скроллер

Для замечания «закрепить колонку ФИО» первая попытка добавила собственный `overflow-x-auto` вокруг таблицы. Замер в браузере показал, что он бесполезен: у него `scrollWidth === clientWidth`, потому что shadcn `<Table>` рендерится внутри собственной обёртки `relative w-full overflow-x-auto`, и прокручивается именно она. Лишняя обёртка убрана, `sticky left-0` работает относительно внутреннего скроллера.

Заодно уточнение к правилу проекта «Pattern F, а не `position: sticky`»: Pattern F (две синхронизированные панели) нужен большим сеткам с переменной высотой строк — там sticky на `<td>` действительно ломается. Для обычной таблицы с однородными строками достаточно `sticky left-0` с непрозрачным фоном; в репозитории это уже было сделано так в `AccessMatrixTable`. Проверять закрепление обязательно замером (`getBoundingClientRect().left` до и после `scrollLeft`), а не глазом: при отсутствии переполнения sticky «работает» и не делает ничего.

### Чужой отчёт «сделано» проверяется кодом, а не доверием

Из 16 пунктов, по которым разработка 01.09 отчиталась как о сделанных или почти сделанных, сверка с кодом прототипа нашла **10 реальных пробелов в 7 пунктах** — включая три из четырёх замечаний по одной строке трекера. При этом в шести пунктах отчёт оказался точен, а в четырёх — прототип был даже лучше прода. То есть ни «всё сделано», ни «ничего не сделано» не были верны, и угадать соотношение заранее было нельзя.

Стоимость сверки — несколько грепов на пункт; стоимость её пропуска — круг переписки с клиентом и потерянная неделя. Отдельно окупилось то, что по каждому пункту проверялась **первопричина**, а не формулировка замечания: «маркетинг видит полный календарь» лечилось не в меню, «нет даты отправки» — не в компоненте, который её показывает.

---

## 2026-09-24 — Promo: жёлтые пункты проверки прода 14–15.09 (№12–27)

### Свой ответ «готово» тоже проверяется кодом — и конкретным сценарием, а не наличием функции

Урок 11.09 был про чужой отчёт. Здесь та же проверка нашла четыре неточности в НАШЕМ ответе от 11.09: кнопки Пн–Вс «убраны» (на деле обязательный шаг формы с 06.08), «Черновик сразу после создания» (только у акции целиком — позиция, добавленная в акцию на согласовании, сразу была видна проверяющим), «в календаре у строки только просмотр» (у КД стояли ✓/✗ по исключениям), «в панели по ФИО только задачи рейтинга» (`matchesMetric("all")` пропускал всё). Общее у всех четырёх: функция в коде есть, но утверждение описывало не её поведение в сценарии клиента, а намерение. Правило: прежде чем писать клиенту «готово», прогнать ровно его сценарий кликами (КМ добавляет позицию в акцию на согласовании → переключиться на КД → что видно?), а не найти в коде похожую ветку.

### Общий `<Button>` под Radix `asChild` — третий рецидив, теперь в чужом коде

Новые `DatePickerField` / `DateRangeFilter` (коммит другого разработчика) обернули `<Button>` в `PopoverTrigger asChild`. Общий `Button` не пробрасывает ref → Popper не находит якорь → контент остаётся в заглушке `translate(0, -200%)`, то есть за верхней кромкой экрана: календарь не открывался ни в одном из 30 полей дат. Признак в консоли — `Function components cannot be given refs … SlotClone … at Button`. Проверка за секунду: `rg -U "(PopoverTrigger|DropdownMenuTrigger|TooltipTrigger|SheetTrigger|DialogTrigger)\s+asChild>\s*\n\s*<Button\b" Promo/src`. Лечение прежнее — нативная `<button>` + `buttonVariants`. Меняя импорт, не удаляй `Button`, пока не проверил остальной файл: в `DateRangeFilter` он ещё жил в футере поповера, и `vite build` это пропустил.

### Promo можно типизировать целиком — `tsc` из scratchpad

В репозитории нет `typescript`, а `vite build` транспилирует без проверки типов. Рабочий рецепт: `npm install typescript@5.6` в scratchpad + `tsconfig.json` там же — `jsx: react-jsx`, `moduleResolution: bundler`, `strict`, `noEmit`, `skipLibCheck`, `baseUrl: D:/Texnomart`, `paths` для `@/*`, `@texnomart/ui/*`, `@texnomart/shared/*` и для `react` / `react/jsx-runtime` / `react-dom` на `node_modules/.pnpm/@types+react@…/node_modules/@types/react/…` (в pnpm-сторе они есть транзитивно), `include` на `Promo/src/**`, плюс `shims.d.ts` с `ImportMeta.env` и `*.svg`/`*.png`. Полная проверка ~1 мин; базовая линия — 3 известные ошибки (`plan-store.ts` ×2, `main.tsx`), всё сверх — новое. В этом раунде `tsc` поймал упомянутый `Button` без импорта, который упал бы только после выбора периода — футер рендерится лениво, и клик по пустому фильтру его не показывал.

### Сценарий, закреплённый за ролью, нужен посев для ЭТОЙ роли

Роль «КМ» в полном календаре — фиксированный km-1, а обе посевные отклонённые позиции основного потока принадлежали другим КМ. Исправленная деривация статуса ничего бы не дала аналитику: в прототипе у «КМ» просто не было отклонённой позиции. Тот же класс, что «уже реализовано ≠ достижимо на демо» (Broker, 19.08). Смежная ловушка: `kmForCampaign` отдавал новую позицию «первому КМ акции», и КМ не видел собственное добавление. Правило: для каждого сценария замечания спросить «под какой ролью / идентичностью его будут проверять и есть ли у неё данные», а не только «работает ли логика».

### Видимость «по типу» не выражает события, адресованные этапу

Маршрутизация уведомлений E-2b шла только по типу, и «поступило на согласование» уходило и старшему КМ, и КД — событие одного этапа видел другой. Лечение без нового поля: у события уже был `visibleTo` («оставлен для обратной совместимости»); видимость стала «тип в настройке роли И роль среди адресатов», одной функцией `roleReceivesNotification` для ленты, колокольчика, блоков Администратора и подписи «Вам как». Вторая половина правки — эмиссии: адресат задаётся по этапу и виду заявки. Правило: если конфиг «кто что получает» описывает категории, а событие может касаться одного исполнителя категории — адресат события нужен отдельно, и все потребители должны читать одно правило.

### У строки бывает история и активный запрос одновременно — «текущий» надо выводить явно

У L-0023 висело отклонённое изменение, а КМ отправил запрос на исключение. Панель «Детали запроса» брала `pending ?? removal…` и показывала тип, автора и дату старого изменения вместо нового запроса. Правило: где строка может нести и прошлое решение, и активный запрос, приоритет «активное важнее исторического» формулируется одной переменной (`exclusionRequest`) и применяется ко всем полям блока, а не угадывается по наличию полей.

### Инструменты: большие правки — скриптом из файла, с точными совпадениями

Bash-heredoc с JSX и кириллицей дважды оборвался на кавычках («unexpected EOF while looking for matching `'`»). Надёжнее: Python-скрипт через Write в scratchpad, каждая замена — с `assert s.count(old) == 1` (скрипт пишет файл только если все замены нашлись), окончания строк сохраняются (`newline=""` + восстановление `\r\n`). Совпадение-подстрока ловит и строку с бо́льшим отступом — якорь брать с соседней строкой.

### Переименовал справочные значения — проверь сохранённое в браузере и раскрытые списки

Посевные категории Promo переведены на девять согласованных, а собственная проверка прошла чисто, потому что шла в чистом профиле и смотрела только закрытое поле фильтра. Независимое ревью нашло два промаха. (1) `promo:category-distribution` перекрывает сид целиком: у любого, кто хоть раз сохранил форму распределения на старом деплое, остались бы старые названия — ровно у аналитика, для которого делалась правка. Сбрасывать ключ (`-v2`) нельзя, пропадут введённые пользователем данные; лечится переводом старых значений при чтении (`LEGACY_CATEGORY_NAMES` в `read()`). (2) Radix Popper ставит обёртке списка `min-width: max-content`, у shadcn `SelectContent` потолка нет — длинный пункт раздвигает список за край экрана на 390px; лечится `max-w-(--radix-select-content-available-width)`, пункты `SelectItem` переносятся сами. Правило: при смене строковых значений в сидах — (а) грепнуть сторы localStorage, которые перекрывают эти сиды, и проверить с подложенными старыми данными; (б) на 390px открыть каждый список, где значение стало длиннее.

## 2026-09-25 — Promo: `/code-review` раунда 24.09

### Замечания ревью уровня high проверяются опровержением до того, как нести их пользователю

Из 10 замечаний `/code-review high` три не выдержали проверки по коду: «вечная корректировка» оказалась только посевным состоянием, «старые уведомления уходят не тем ролям» — осиротевшими ссылками на заявки, которых после перезагрузки уже нет, рассинхрон полосы прокрутки — невозможным (полоса снимается только без переполнения). Проверяющие с задачей опровергнуть, по одному на замечание, заодно дали воспроизведение кликами для подтверждённых. Правило: high-ревью — список гипотез; к пользователю идут только подтверждённые, с путём воспроизведения и оценкой «внесено раундом или было».

### Необязательное сужающее поле + значение по умолчанию = мёртвая настройка

Правило видимости «тип в настройке роли И роль среди адресатов события» верно, пока адресат задан только там, где он зависит от события. Фабрика подставляла аудиторию по умолчанию каждому событию — и настройка Администратора перестала расширять что-либо (69 из 120 переключателей). Правило: если конфиг решает «кто получает», поле-сужение (`visibleTo`) не должно иметь значения по умолчанию; ставить его только событиям этапа и личным обращениям.

### Синтезированная запись о решении — не правка данных

Отказ по исключению хранился как `pending` вида «изменение» с полем `removed`, чтобы переиспользовать панель отклонений. Всё, что умеет «копить» и «применять» правки (`mergePendingChange`, `patchFrom`), приняло это поле за данные: согласование цены исключало позицию. Правило: если запись решения переиспользует структуру правки, пути накопления и применения должны её распознавать (`isRejectedExclusion`), а поле-маркер — не применяться никогда; и все поля активного запроса со строки при отказе снимать, иначе их подхватят запасные значения отображения.
