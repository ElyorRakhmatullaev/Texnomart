  ...
  src/
    ...
    app/
      ...
      components/
        ...
        approvals/                  # S3 — Согласование и проверка (Phases 1–2; Phase 3 pending)
          ApprovalsProvider.tsx     #   review store (useReducer) ABOVE the queue + detail routes; actions approve/reject; selectors getItem/queueFor
          ApprovalsPage.tsx         #   role-aware review queue: PageHeader + FilterBar (тип/КМ/«только просроченные»); reviewers get the queue, others an explainer
          ReviewQueueTable.tsx      #   Pattern C table (№ промо/тип/название/КМ/отправлено/статус/SLA-таймер + OverdueTag) → Mode-B MobileListCard list below md
          ApprovalDetailPage.tsx    #   /approvals/:id split view (DetailPageHero + SLA InfoRows; lg:grid-cols-[1fr_340px], sticky actions); hosts the reject ReasonDialog + toasts, canAct gating
          ...
          ReviewActionsPanel.tsx    #   «Согласовать всё» / «Отклонить выбранные» / «Отклонить весь набор»; read-only note per terminal/wrong-role state; «Комментарии проверки» log
...
| `/approvals` | ApprovalsPage | **In progress (S3 — Phases 1–2)** — role-aware review queue (Pattern C / Mode-B) with working-days SLA timer |
| `/approvals/:id` | ApprovalDetailPage | **In progress (S3 — Phases 1–2)** — split view: submitted snapshot + per-line/bulk approve·reject, status transitions, comment log |
