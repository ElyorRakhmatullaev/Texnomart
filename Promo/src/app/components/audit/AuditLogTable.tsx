...
function ObjectCell({ event }: { event: AuditEvent }) {
  return (
    <div className="min-w-0">
      <Badge
        variant="outline"
        className="mb-0.5 text-[10px] font-normal text-gray-500"
      >
        {AUDIT_OBJECT_LABEL[event.objectType]}
      </Badge>
      <p className="truncate text-sm text-gray-900" title={event.objectLabel}>
        {event.objectLabel}
      </p>
      {event.campaignId && (
        <p className="font-mono text-[11px] text-gray-400">{event.campaignId}</p>
      )}
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="hidden md:block">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <AuditLogFilters
            ...
            roles={roles}
          />
          <div className="ml-auto pb-0.5 text-xs text-gray-500">
      ...
      <div className="flex items-center justify-between gap-2 md:hidden">
        ...
        >
          ...
          Фильтры
          {activeCount && (
            <span className="ml-0.5 size-2 rounded-full bg-[#FFD60A]" />
          )}
        </Button>
        ...
        <>
                ...
                <TableBody>
                  {filtered.map((e) => (
                    <TableRow key={e.id} className="align-top">
                      ...
                      <TableCell className="tabular-nums text-sm whitespace-nowrap text-gray-700">
          ...
          <div className="flex flex-col gap-2.5 md:hidden">
            {filtered.map((e) => (
              <div
                key={e.id}
                className="rounded-lg border border-gray-200 bg-white p-3 shadow-[0px_2px_4px_rgba(204,204,204,0.25)]"
              >
                ...
                {e.comment && (
                  <p className="mt-2 text-xs text-gray-600">{e.comment}</p>
                )}
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-gray-200 bg-white py-16 text-center">
      <FileSearch className="size-12 text-gray-300" />
      <div>
        <p className="font-medium text-gray-900">Записей не найдено</p>
