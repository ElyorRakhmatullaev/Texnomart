...
- **TypeScript в `Broker/` отсутствует** — ни пакета `typescript`, ни `tsconfig.json`; `vite build` транспилирует через esbuild **без проверки типов**. Зелёная сборка не доказывает почти ничего: удалённый экспорт, обращение к несуществующему полю состояния и несогласованный `Record<...>` компилируются молча. Сигнатуры сверять глазами по блоку **Interfaces** своей задачи.
- **Греп вместо сборки как гейт удаления.** Любая задача, которая удаляет или переименовывает символ, обязана закончиться грепом по `Broker/src` на имена удалённых символов с пустым результатом. Сборка при этом всё равно запускается — она ловит синтаксис и незакрытые импорты модулей, но не ссылки на исчезнувшие имена.
...
- [ ] **Step 1b: Долги по `DemoScenarioBar` из задачи 5**

Три правки в том же файле, пока он открыт:

1. **Цель касания.** Пилюли демо-панели сейчас `rounded-full px-3 py-1 text-xs` — это около 24 px по высоте против минимума 44 px из `.claude/rules/design.md` (Pattern K). Мок показывают с планшета, там это не нажать. Добавить `min-h-11` каждой кнопке панели, включая «Сессия истекла», и `items-center`, чтобы текст остался по центру.
2. **Мёртвый интерфейс.** `DemoOption` (`{ id, label }`) экспортируется, но не используется ни разу: массив `options` типизирован инлайн как `{ label; active; onSelect }[]` и не имеет поля `id`. Удалить `DemoOption`.
3. **Честный предикат.** `active: state.alifLimitStatus !== "rejected"` возвращает `true` и для `"pending"`. Заменить на `=== "ready"` — состояние `"pending"` до этой панели не доходит, но предикат не должен обещать больше, чем проверяет.
