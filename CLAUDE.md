...
| **Client Broker** | `Broker/` | Client Broker — операторское приложение скоринга в магазине (ветка Alif: верификация клиента+карты → фото/MyID → выбор рассрочки → холд предоплаты → доп. данные → финал) | Active — переработан по ТЗ 18.08: верификация+карты с OTP-модалом, фото/MyID (размер фото не проверяется — ждём ответа Alif/MyID), холд предоплаты, финал с OTP-модалом и договором (2026-08-18); ветка Alif перенесена в попап на «Выборе рассрочки» по фрейму 32 (2026-08-18); QA 9/9 + финальные фиксы (заморозка срока после подтверждения); 14 экранов мока сняты в Figma — страница «Мок 18.08 — Alif (из кода)»; **ошибочные и краевые пути закрыты по чек-листу PM (2026-08-19)** — достижимые отказы фото/MyID через демо-переключатель, лимит попыток OTP + срок жизни кода, видимый статус «Холд отменён» + отмена холда с любой поздней фазы (`cancelOffer` отделён от `holdCancel`, `checkoutPhaseOf` переупорядочен), «Открыть договор» как постоянный вход к договору; 11 новых состояний сняты в Figma — страница «Мок 19.08 — Alif: ошибки и краевые состояния (из кода)»; **⏳ ждёт третьей итерации** — ТЗ по API Alif (`alif_ui_tz.md`, 19.08, 8 экранов, не реализовано; дельта — в `Broker/CLAUDE.md`) |
