...
│   ├── package.json
│   ├── vite.config.ts
│   ├── public/                 # contract-mock.pdf
│   └── src/                    # app/ (shell, routes, scoring-flow, components/{shell,scoring,checkout,alif}),
│                               # lib/ (broker-mock-data + alif-application — чистая логика заявки, без React)
...
pnpm dev:broker                 # Start Client Broker dev server
pnpm build:broker               # Build Client Broker
...
All three apps deploy to GitHub Pages via `.github/workflows/deploy.yml` (push to `main` + manual `workflow_dispatch`), under subpaths on one site (no root landing page):
...
| **Client Broker** | `https://elyorrakhmatullaev.github.io/Texnomart/broker/` |

The workflow builds each app with a `BASE_PATH` env (`/Texnomart/dashboard/`, `/Texnomart/promo/`, `/Texnomart/broker/`), assembles `_site/{dashboard,promo,broker}` + a shared root `404.html`, and publishes via `actions/deploy-pages`.
...
- Supported languages (per-app, selector exists): **Dashboard** — RU, O'zbek (Лат.) only (Cyrillic dropped per client feedback); **Promo** — RU, O'zbek (Кирилл.), O'zbek (Лат.); **Broker** — статичная пара «RU | UZ» в собственном топ-баре (display-only, вне AppShell). The AppShell language list is configurable via `AppShellConfig.languages` (defaults to all three when omitted; the selector is display-only — no runtime i18n yet).
...
- `Broker/CLAUDE.md` — Client Broker routes, mock conventions, Alif flow state (попап оформления + краевые пути 18–19.08 — complete; § «Pending» — дельта по ТЗ на основе API Alif)
