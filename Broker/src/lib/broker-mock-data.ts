...
export const BANKS: Bank[] = [
  {
    ...
    tenors: [3, 6, 12, 18, 24],
    defaultTenor: 6,
    // Финансируемая сумма — ORDER.amount (10 000 000) минус предоплата
    // (1 000 000) = 9 000 000 — лимит обязан её покрывать на любом сроке,
    // иначе карточка обещает «✓ Одобрена» под лимитом меньше суммы покупки.
    limit: 12_000_000,
...
// Форма сохранена (длиннее срок → ниже лимит), но каждое значение поднято
// выше финансируемой суммы (ORDER.amount − ALIF_PREPAYMENT = 9 000 000) —
// иначе любой план показывал бы лимит меньше суммы покупки под зелёным
// «✓ Одобрена».
export const ALIF_LIMITS: AlifLimitSeed[] = [
  { duration: 3, amount: 12_000_000 },
  { duration: 6, amount: 12_000_000, promo: "Без переплаты первые 2 мес." },
  { duration: 12, amount: 10_500_000 },
  { duration: 18, amount: 9_800_000 },
  { duration: 24, amount: 9_200_000 },
]
