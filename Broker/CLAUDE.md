...
| `/scoring/alif/info` | InstallmentInfoPage | 5 «Информация по рассрочке» | `additionalData` saved |
| `*` | — | — | redirect → `/scoring/verification` |

Every guard failure redirects to `/scoring/verification` (not to an intermediate step) — direct deep-links without state fall all the way back to the start. The seeded Alif `prepayment` is `1_000_000` (non-zero), so `/scoring/alif/hold` is always on the path to Alif; the `prepayment === 0` guard clause exists for a future bank/seed where the hold step is skippable, but is not currently exercised by any seed.
...
4. **`/scoring/alif/hold` — Предоплата по рассрочке** (only reached because Alif's seeded `prepayment > 0`; step 3 in the stepper, no dedicated step of its own). Shows the prepayment amount (**1 000 000 сум**), the debit card (first confirmed card, masked `9860 •••• 1296`), and an explanation that funds are held then charged on credit confirmation. «Подтвердить удержание» → `holdHold()` → `held` spinner state («Предоплата удерживается…») → after **~2 с** (`PREPAYMENT_HOLD_DELAY_MS`) auto-`holdConfirm()` → `confirmed` badge + «Продолжить» → `/scoring/alif/details`. «Отменить холд» (hidden while `held`) → `holdCancel()` + toast «Холд отменён» + back to `/scoring/banks`; `alifSelected`/`tenor` are **not** reset, so the bank stays selected.
...
- **Card validation** (`VerificationPage`, before the OTP modal ever opens): number ending `CARD_NOT_FOUND_SUFFIX = "0000"` → «Карта не найдена»; ending `CARD_BLOCKED_SUFFIX = "9999"` → «Карта заблокирована» (AlertDialog, no OTP step).
- **Seeded card**: `SEED_CARD = { mask: "9860 3569 7266 1296", expiry: "11/29" }` — pre-confirmed in `INITIAL` state.
- **Seeded client**: `BROKER_CLIENT` (Артем Борисов, `+998 94 983 98 48`, ПИНФЛ `2116358415458`, паспорт `AD 1276543`).
